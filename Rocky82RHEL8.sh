...
if [ -f kupdate.txt ]; then
    echo "Restarting"
        ...
        read -r -p "Lets download some files [y/N] " response
...
if [ -f kupdate.txt ]; then
        ...
        if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]
        then
                sudo yum install kernel-core-4.18.0-513.11.1.el8_9 -y
                echo "kernel-core-4.18.0-513.11.1.el8_9" > kupdate.txt
